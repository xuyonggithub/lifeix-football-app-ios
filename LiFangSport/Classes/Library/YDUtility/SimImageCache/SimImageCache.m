    ...
    if (shouldCache) {
        ...
        if (!image) {
            ...
            if (filePath.length == 0) {
                if (!isHighResolution && ![noExtFileName hasSuffix:@"@2x"]) {
                    //非高清的图片补充
                    noExtFileName = [noExtFileName stringByAppendingString:@"@2x"];
                    filePath = [self existPathForNoExtName:noExtFileName ext:[name pathExtension]];
                }
                else if(isHighResolution && [noExtFileName hasSuffix:@"@2x"]){
                    noExtFileName = [noExtFileName substringToIndex:noExtFileName.length-3];
                    filePath = [self existPathForNoExtName:noExtFileName ext:[name pathExtension]];
                }
            }
            if (filePath) {
